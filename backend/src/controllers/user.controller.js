...
export async function getRecommendedUsers(req,res) {
    try {
        ...
        const recommednedUsers = await User.find({
            $and: [
        ...
        res.status(200).json(recommednedUsers);
    } catch (error) {
...
export async function acceptFriendRequest(req,res) {
    try {
        ...
        // $addToSet ensures that the user is only added if they are not already in the array
...
export async function getFriendRequests(req,res) {
    try {
        const incomingReqs = await FriendRequest.find({
            ...
            status:"pending",
        }).populate("sender", "fullName profilePic nativeLanguage, learningLanugage");
